...
class Death:
  ...
  def __init__(self, pmanager):
    ...
    self.ways_to_die = ['{} dies in their sleep', '{} is lost in the night','{} drowned']

    # Aging related deaths
    self.old_age = ['{} had a heart attack']

    # Work related deaths
    self.hunter_death = ['{} was lost on a hunt','{} was mauled to death by bears']
  ...
  def kill_villager(self, villager, reason=''):
    ...
    if reason == '':
      ...
      if villager.job == 'Woodcutter': reasons += self.woodcutter_death
      if villager.job == 'Miner': reasons += self.hunter_death
      if villager.job == 'Hunter': reasons += self.hunter_death
      if villager.job == 'Farmer': reasons += self.farmer_death
      reason = r.choice(reasons)

    self.log.append([2, '\u001b[31;1m' + reason.format(villager.name) + '\u001b[0m'])
