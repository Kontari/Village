...
class Mood:
    ...
    def death_event(self, rel_strength, s_name, o_name, txt=''):
        ...
        if rel_strength < 1.00:
            self.__mood_event(
                1, 0, 2, txt='{} is glad {} died.'.format(s_name, o_name))
        elif 1.00 < rel_strength < 2.00:
            self.__mood_event(
                0, 1, 1, txt='{} is indifferent to {}s death.'.format(s_name, o_name))
        elif 2.00 < rel_strength < 4.00:
            self.__mood_event(
                0, 1, 3, txt='{} is hurt over {}s death.'.format(s_name, o_name))
        elif 4.00 < rel_strength:
            self.__mood_event(-2, 2, 10,
                              txt='{} is profoundly damaged over {}s death.'.format(s_name, o_name))
