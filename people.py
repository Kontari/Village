...
class PeopleManager:
  ...
  def settlers(self):
    ...
    if r.randint(configGlobals.SETTLER_CHANCE, 360) == 360:
      ...
      self.log.append([0,'{}  new settlers have arrived!'.format(num_settlers)])
  ...
  def children(self):
    ...
    if r.randint(1,20) > 19:
      ...
      self.log.append([0,'{} has appeared!'.format(self.people[-1].name)])
  ...
  def sort_people(self, sort_by):
    ...
    if sort_by == 'job':
      ...
      '''
      radix = [['Woodcutter'],['Miner'],['Hunter'],['Farmer'],['Child'],['Infant'],['None'],['']]
...
class Person:
  ...
  def check_marriage(self):
    ...
    if (self.romance == False) and (18 < self.age < 50) and (self.spouse == ''):
      ...
      self.log.append([2,'{} ({}) is looking for a partner.'.format(self.name, self.gender[0])])
  ...
  def show_stats(self):
    ...
    basic_info = '{}  {}  Age:{} {} {}'.format('{0: <10}'.format(self.name) , '{0: <20}'.format(job), self.age, symb, self.mood.mood)
