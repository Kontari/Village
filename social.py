...
class SocialEvents:
  ...
  def __init__(self, people_objs):
    ...
    self.places = ['street','store','well','bar','bakery','butchery','town square','forest','gardens']

    self.verbs = ['runs into', 'meets','talks with','hangs with','spots']

    self.negative_verbs = ['spits on', 'gets into a fight with', 'attacks', 'insults']
  ...
  def negative_event(self, p_one, p_two):
    ...
    event_text = (r.choice(self.disliked_events)).format(p_one.name, r.choice(self.negative_verbs),p_two.name, r.choice(self.places))
  ...
  def positive_event(self, p_one, p_two):
    ...
    event_text = (r.choice(self.friendly_events)).format(p_one.name, r.choice(self.verbs),p_two.name, r.choice(self.places))
